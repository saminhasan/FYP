...
def genrate_mission(path):
	...
	for i in range(len(lla)):
		cmds.add(Command( 0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0, 0, 0, 0, 0, 0, float(lla[i][0]), float(lla[i][1]), -float(lla[i][2])))
