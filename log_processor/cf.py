...
def calc_cost(path, obstacles, verbose=False):
    ...
    if verbose:
        print("Path Length( Theoritical ) : ", length, ' meters.')
    ...
    if verbose:
        print("Minimum Time ( Theoritical ) : ", time, ' seconds.')

    energy_consumption = 0  # calc_energy_consumption(path)
    energy_weight = 1.0
    energy_cost = energy_consumption * energy_weight
    if verbose:
        print("Energy Consumption ( Theoritical ): ", 25 * calc_energy_consumption(path), "joules.")
